  ...
  const exactStats = !search
    ? await sql`
        ...
        SELECT
          ...
          count(*) FILTER (WHERE status = 'neutral_reply')::int as neutral_reply_count,
          count(*) FILTER (WHERE status = 'neutral_reply' AND is_read IS TRUE)::int as neutral_reply_read_count,
          count(*) FILTER (WHERE status = 'neutral_reply' AND is_read IS NOT TRUE)::int as neutral_reply_unread_count,
          count(*) FILTER (WHERE status = 'real_estate_new')::int as real_estate_new_count,
          count(*) FILTER (WHERE status = 'real_estate_new' AND is_read IS TRUE)::int as real_estate_new_read_count,
          count(*) FILTER (WHERE status = 'real_estate_new' AND is_read IS NOT TRUE)::int as real_estate_new_unread_count,
          count(*) FILTER (WHERE status = 'real_estate_promising')::int as real_estate_promising_count,
          count(*) FILTER (WHERE status = 'real_estate_promising' AND is_read IS TRUE)::int as real_estate_promising_read_count,
          count(*) FILTER (WHERE status = 'real_estate_promising' AND is_read IS NOT TRUE)::int as real_estate_promising_unread_count,
          count(*) FILTER (WHERE status = 'real_estate_neutral')::int as real_estate_neutral_count,
          count(*) FILTER (WHERE status = 'real_estate_neutral' AND is_read IS TRUE)::int as real_estate_neutral_read_count,
          count(*) FILTER (WHERE status = 'real_estate_neutral' AND is_read IS NOT TRUE)::int as real_estate_neutral_unread_count,
          count(*) FILTER (WHERE status = 'contacted')::int as contacted_count,
          count(*) FILTER (WHERE status = 'contacted' AND is_read IS TRUE)::int as contacted_read_count,
          count(*) FILTER (WHERE status = 'contacted' AND is_read IS NOT TRUE)::int as contacted_unread_count,
          count(*) FILTER (WHERE status = 'quoted')::int as quoted_count,
          count(*) FILTER (WHERE status = 'quoted' AND is_read IS TRUE)::int as quoted_read_count,
          count(*) FILTER (WHERE status = 'quoted' AND is_read IS NOT TRUE)::int as quoted_unread_count,
          count(*) FILTER (WHERE status = 'scheduled')::int as scheduled_count,
          count(*) FILTER (WHERE status = 'scheduled' AND is_read IS TRUE)::int as scheduled_read_count,
          count(*) FILTER (WHERE status = 'scheduled' AND is_read IS NOT TRUE)::int as scheduled_unread_count,
          count(*) FILTER (WHERE status = 'won')::int as won_count,
          count(*) FILTER (WHERE status = 'won' AND is_read IS TRUE)::int as won_read_count,
          count(*) FILTER (WHERE status = 'won' AND is_read IS NOT TRUE)::int as won_unread_count,
          count(*) FILTER (WHERE status = 'sold')::int as sold_count,
          count(*) FILTER (WHERE status = 'sold' AND is_read IS TRUE)::int as sold_read_count,
          count(*) FILTER (WHERE status = 'sold' AND is_read IS NOT TRUE)::int as sold_unread_count,
          count(*) FILTER (WHERE status = 'lost')::int as lost_count,
          count(*) FILTER (WHERE status = 'lost' AND is_read IS TRUE)::int as lost_read_count,
          count(*) FILTER (WHERE status = 'lost' AND is_read IS NOT TRUE)::int as lost_unread_count,
          count(*)::int as total,
          coalesce(sum(job_value) FILTER (WHERE status IN ('won', 'sold')), 0)::numeric as total_revenue,
          coalesce(sum(quoted_amount) FILTER (WHERE status IN ('quoted','scheduled')), 0)::numeric as pipeline_value
        FROM filtered
